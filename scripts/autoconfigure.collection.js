...
function combineFields(a, b) {
  ...
  if (a.widget === b.widget) {
    if (a.fields && b.fields) {
      ...
      a.fields.forEach(field => {
        newFields[field.name] = combineFields(
          field,
          b.fields.find(f => f.name === field.name),
        );
      });
  ...
  return [a, b].sort((fieldA, fieldB) =>
    compareWidget(fieldB.widget, fieldA.widget),
  )[0];
}
...
const extension = Object.keys(extensions).sort(
  (a, b) => extensions[b] - extensions[a],
)[0];
