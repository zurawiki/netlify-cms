...
const hasProperty = (config, property) =>
  ({}.hasOwnProperty.call(config, property));
      ...
      return OrderedMap().withMutations(map => {
        (configCollections || []).forEach(configCollection => {
          if (hasProperty(configCollection, "folder")) {
            ...
            throw new Error(
              "Unknown collection type. Collections can be either Folder based or File based. Please verify your site configuration",
            );
          }
  ...
  [FOLDER]: {
    entryExtension(collection) {
      return (
        ...
        formatToExtension(collection.get("format") || "markdown")
      );
    },
    fields(collection) {
