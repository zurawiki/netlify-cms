...
export default class API {
  ...
  isUnpublishedEntryModification(path, branch) {
    return this.readFile(path, null, branch).then(data => true).catch(err => {
    ...
    return this.request(`${this.repoURL}/git/refs/heads/cms`)
      .then(branches =>
        filterPromises(branches, branch => {
          ...
          return this.request(
            `${this.repoURL}/pulls?head=${branchName}&state=open`,
          ...
          this.user()
            ...
            .then(username =>
              this.storeMetadata(contentKey, {
                ...
                description: options.parsedData &&
                  options.parsedData.description,
                objects: {
                  entry: {
                    path: entry.path,
                    sha: entry.sha,
        ...
        .then(response => {
          ...
          return this.user()
            ...
            .then(updatedMetadata =>
              this.storeMetadata(contentKey, updatedMetadata),
            )
  ...
  getTree(sha) {
    return sha
      ? this.request(`${this.repoURL}/git/trees/${sha}`)
  ...
  uploadBlob(item) {
    const content = item instanceof AssetProxy
      ? item.toBase64()
      : this.toBase64(item.raw);
