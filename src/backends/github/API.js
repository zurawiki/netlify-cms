    ...
    return this.request(`${this.repoURL}/git/refs/heads/cms`)
      .then(branches =>
        filterPromises(branches, branch => {
          ...
          return this.request(`${this.repoURL}/pulls?head=${branchName}&state=open`).then(prs =>
            prs.some(pr => pr.head.ref === branchName),
          );
        }),
      )
      .catch(error => {
  ...
  editorialWorkflowGit(fileTree, entry, filesList, options) {
    ...
    if (!unpublished) {
      ...
      return this.getBranch()
        ...
        .then(prResponse =>
          this.user().then(user => (user.name ? user.name : user.login)).then(username =>
