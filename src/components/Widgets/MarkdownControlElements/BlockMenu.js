...
export default class BlockMenu extends Component {
  ...
  buttonFor(plugin) {
    return (
      <li key={`plugin-${plugin.get("id")}`}>
        <button onClick={this.handlePlugin(plugin)}>
          {plugin.get("label")}
        </button>
      </li>
    );
  ...
  render() {
    ...
    return (
      <div className={classNames.join(" ")} ref={this.handleRef}>
        <button className={styles.button} onClick={this.handleToggle}>+</button>
        <ul
          className={[
            styles.menu,
            isExpanded && !openPlugin ? styles.expanded : styles.collapsed,
          ].join(" ")}
        >
