...
function getCleanPaste(e) {
  ...
  return new Promise(resolve => {
    const isHTML = !!Array.from(transfer.types).find(
      type => type === "text/html",
    );
    ...
    if (isHTML) {
      ...
      if (!data.match(/^\s*<!doctype/i)) {
        ...
        div.setAttribute(
          "style",
          "opacity: 0; overflow: hidden; width: 1px; height: 1px; position: fixed; top: 50%; left: 0;",
        );
...
export default class RawEditor extends React.Component {
  ...
  surroundSelection(chars) {
    ...
    if (regexp.test(selection.selected)) {
      changed = selection.selected.substr(
        chars.length,
        selection.selected.length - chars.length * 2,
      );
  ...
  handleSelection = () => {
    ...
    if (
      ...
      !HAS_LINE_BREAK.test(selection.selected)
    ) {
      try {
        const selectionPosition = this.caretPosition.get(
          selection.start,
          selection.end,
        );
        this.setState({
  ...
  handlePaste = e => {
    ...
    getCleanPaste(e).then(paste => {
      ...
      newSelection.start = newSelection.end =
        beforeSelection.length + paste.length;
      this.newSelection = newSelection;
