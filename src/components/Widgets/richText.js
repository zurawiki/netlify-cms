...
function processEditorPlugins(plugins) {
  ...
  plugins.forEach(plugin => {
    const basicRule = MarkupIt.Rule(
      plugin.id,
    ).regExp(plugin.pattern, (state, match) => ({
      data: plugin.fromBlock(match),
    }));

    const markdownRule = basicRule.toText(
      (state, token) => `${plugin.toBlock(token.getData().toObject())}\n\n`,
    );
      ...
      return (
        <div {...props.attributes} className={className}>
          ...
          <div className="plugin_fields" contentEditable={false}>
            {plugin.fields.map(
              field => `${field.label}: “${node.data.get(field.name)}”`,
            )}
          </div>
        </div>
      );
    ...
    augmentedMarkdownSyntax = augmentedMarkdownSyntax.addInlineRules(
      markdownRule,
    );
...
function processAssetProxyPlugins(getAsset) {
  const assetProxyRule = MarkupIt.Rule(
    "assetproxy",
