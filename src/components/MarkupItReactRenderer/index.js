      ...
      <pre>
        <code
          ...
          dangerouslySetInnerHTML={{
            __html: token.get("tokens").map(token => token.text).join(""),
          }}
        />
      </pre>
    );
...
export default class MarkupItReactRenderer extends React.Component {
  ...
  sanitizeProps(props) {
    ...
    if (props.image) {
      props = Object.assign({}, props, {
        src: getAsset(props.image).toString(),
      });
    }
  ...
  renderToken(schema, token, index = 0, key = "0") {
    ...
    if (plugin) {
      ...
      return typeof output === "string" ? <span dangerouslySetInnerHTML={{ __html: output }} /> : output;
    }
...
MarkupItReactRenderer.propTypes = {
  ...
  schema: PropTypes.objectOf(PropTypes.oneOfType([PropTypes.string, PropTypes.func])),
