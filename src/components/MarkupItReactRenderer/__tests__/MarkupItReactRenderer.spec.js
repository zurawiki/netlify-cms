...
describe("MarkitupReactRenderer", () => {
  describe("basics", () => {
    it("should re-render properly after a value and syntax update", () => {
      const component = shallow(<MarkupItReactRenderer value="# Title" syntax={markdownSyntax} getAsset={getAsset} />);
...
###### H6
`;
        const component = shallow(<MarkupItReactRenderer value={value} syntax={markdownSyntax} getAsset={getAsset} />);
      ...
      for (const heading of [...Array(6).keys()]) {
        it(`should render Heading ${heading + 1}`, () => {
          ...
          const component = shallow(
            <MarkupItReactRenderer value={value} syntax={markdownSyntax} getAsset={getAsset} />,
          );
          expect(component.html()).toMatchSnapshot();
        });
      }
...
`;
      const component = shallow(
        <MarkupItReactRenderer value={value} syntax={myMarkdownSyntax} schema={myCustomSchema} getAsset={getAsset} />,
      );
      expect(component.html()).toMatchSnapshot();
    });
  });
  ...
  describe("HTML rendering", () => {
    it("should render HTML", () => {
      ...
      const component = shallow(<MarkupItReactRenderer value={value} syntax={htmlSyntax} getAsset={getAsset} />);
      expect(component.html()).toMatchSnapshot();
    });
  });
});
